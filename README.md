...
v0.1.0-alpha.1 是一个可运行的内部工程候选：普通用户输入一句能力请求，系统在三个独立本机进程之间自动发现、调用 OPP 协商契约、验证权限、建立会话、经 A→B→C 传输并返回带证据的执行结果。
...
`verify` 冻结源码哈希，按文件顺序运行全部测试，然后分别启动 UDP 和 TCP 三进程见证，保存签名回执、磁盘账本和有界计时到 `evidence/LOCAL_VERIFICATION.json`。重放并发测试内部仍使用真实并发。普通 `npm test` 可并行运行文件；发行证据用顺序执行避免故障注入互相干扰。
...
5. 固定 RCL 程序实际编译执行事务准入、会话迁移和路由约束；Node/Python 负责观测、传输与专业运行。
...
阅读 `docs/REALITY_AUDIT.md`、`docs/CANONICAL_OWNERSHIP.md`、`docs/SPEC_DEVIATIONS.md`、`evidence/INTEGRATION_COURT.md` 和 `evidence/EVIDENCE_LEDGER.json`。原规范保留在 `constitution/`，未修改下载文件或同步项目参考文件。
...
磁盘账本可校验，但密钥与控制面会话尚未做重启恢复。单个协调器负责证据顺序；这不是分布式共识。断连节点不能即时获知撤销，现有短期租约到期提供局部底线。自动重试仅用于本版纯只读计算，不能推导出高影响外部动作的恰好一次执行或补偿事务能力。
...
下一最短缺口是保留同一主体与权限底线的持久重启恢复及隔离后撤销对账，随后才是经成熟加密承载的两台真实设备验证。详见 `docs/NEXT_GAP.md`。
