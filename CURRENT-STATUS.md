...
当前已合并的 alpha.14 继续保持上述本机可恢复 TLS 证据。下一候选 `codex/tinp-transport-policy-v02` 在不改变 OPP Owner 的前提下增加只读 HTTPS/JSON transport policy 和 content-rooted receipt：精确 host/path allowlist、GET、单次请求、无 ambient proxy/credentials、无 redirect，并用显式 `responseFields` 产生 projected response root。GitHub `/repos/xingxuling/OPP` 的实际请求已返回 `PASS`/HTTP 200；它仍只是 TINP provider/transport 候选，不解除公网、跨主机、生产凭据或外部 Owner 边界。
