...
TINP v0.1.0-alpha.17：可选外部 AAF 操作员签章、外部恢复锚点、authority registry 快照、多镜像分发 quorum、历史收敛桥接、本机历史 store、外部收敛见证、跨进程 public-history replay、TCP 与 TLS 1.3 TINP DATA loopback state transfer、可恢复分块传输 journal、策略绑定只读 OPP HTTP 观察适配器的本机候选。
...
alpha.18 候选增加 `scripts/opp-http-consumer-bridge.mjs` 文件输入 CLI：复用 alpha.17 的 plan/policy/request/observation/contract 校验并输出 bounded acceptance receipt，不重新发起网络请求。验证通过后再合并到默认分支；外部第三方 consumer、生产 authority 和 K400 仍保持未证明。
