...
const tracked=['src','adapters','rcl','tests','vendor','shared-ir','registry'].filter(x=>fs.existsSync(x)).flatMap(x=>inventory(x));
...
const summary={format:'twni.local-verification.v0.1',status:'VERIFIED_LOCAL_CANDIDATE',startedAt,finishedAt:new Date().toISOString(),
  ...
  tests:{command:result.command,exitCode:result.code,count:Number(result.stdout.match(/# tests (\d+)/)?.[1]),passed:Number(result.stdout.match(/# pass (\d+)/)?.[1]),failed:Number(result.stdout.match(/# fail (\d+)/)?.[1]),
    stdout:'evidence/tests.tap',stdoutSha256:sha(Buffer.from(result.stdout)),stderr:'evidence/tests.stderr.txt'},
  scenarios,sourceFiles:tracked,sourceTreeRoot:sha(Buffer.from(JSON.stringify(tracked))),
  ...
  boundaries:['Ephemeral local trust fixture; no production identity enrollment or TLS confidentiality',
    ...
    'Single coordinator evidence authority; no distributed consensus, crash recovery of private keys, or global revocation convergence',
