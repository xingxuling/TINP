...
- Candidate branch: `codex/tinp-transport-policy-v01@a9e10b9334cda09f6f3834eae632472fb568db28`.
...
- the known Node `NODE_USE_ENV_PROXY`, `NODE_OPTIONS=--use-env-proxy` and `--use-env-proxy` paths fail closed before transport;
- the default path uses `node:https.request` with a per-request explicit `https.Agent`, and a separate process check succeeded while `node:http.setGlobalProxyFromEnv` pointed at an unavailable loopback proxy.

## Evidence

- Machine evidence: [OPP_HTTP_READONLY_HARDENING_2026-09-11.json](../evidence/OPP_HTTP_READONLY_HARDENING_2026-09-11.json)
- Live public read: [OPP_HTTP_READONLY_NATIVE_2026-09-11.json](../evidence/OPP_HTTP_READONLY_NATIVE_2026-09-11.json)
- Native proxy-boundary check: [OPP_HTTP_READONLY_NATIVE_PROXY_CHECK_2026-09-11.json](../evidence/OPP_HTTP_READONLY_NATIVE_PROXY_CHECK_2026-09-11.json)
...
- Native live receipt root: `d16b35f2fbf660d52c58fcda7b4148a9a8d98300e0955480ae5e2aa871ca4b34`; projected response root remains `a97a84ea4b0b782a102fd59c7cae8e03368271e0430a77da848553cbfe131130`.

## Tests

- Adapter tests: `10/10 PASS`.
- Full TINP suite: `184/184 PASS`, `0` failed, `0` cancelled.
...
- The default native HTTPS path does not depend on the standard Node global proxy Agent; the bounded loopback-proxy experiment is recorded separately.
...
- No cross-host, proxy-support, credential-custody, certificate-lifecycle, SLA or production claim; arbitrary runtime hooks and caller-injected transports remain outside the check.
- No authority lease, RCL Core change or K400 promotion.

## Remaining Production Gaps

`OPP-TRANSPORT-001` remains open: the adapter is unmerged, locally tested and not independently reviewed; external credential/certificate lifecycle, cross-host fault behavior, arbitrary runtime-hook behavior and recovery remain outside the evidence.
