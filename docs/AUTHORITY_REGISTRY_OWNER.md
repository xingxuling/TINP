...
alpha.8 adds a bounded, opt-in authority registry adapter, an offline multi-mirror distribution bundle and a caller-supplied historical convergence bundle. They are integration profiles for TINP host recovery and operator tooling. They are not a new RCL primitive, an RNCS world grant, an AAF replacement, an online transparency log or a production identity service.
