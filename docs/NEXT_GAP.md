...
当前未发布候选 `codex/tinp-transport-policy-v01@a9e10b9` 在不改变 OPP Owner 的前提下增加只读 HTTPS/JSON transport policy 和 content-rooted receipt。策略绑定 vendored OPP `f7b76582a720d9d18af5153affa0fc2d78bc0410`、精确 host/path、GET、单次请求、无 ambient proxy/credentials、无 redirect，并用显式 `responseFields` 产生 projected response root；默认路径使用 Node 内置 `https.request` 与每请求显式 Agent，不依赖全局 fetch dispatcher；GitHub `/repos/xingxuling/OPP` 的实际请求已返回 `PASS`/HTTP 200。标准 Node 全局 proxy API 指向不可用 loopback 时的独立检查也得到 HTTP 200。下一步是把该 native receipt 通过显式 OPP consumer bridge 复验，不能将 provider PASS 改称 OPP 通用第三方互操作或生产网络能力。
