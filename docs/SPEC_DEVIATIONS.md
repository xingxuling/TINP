...
## 14. Resumable TLS DATA transfer is a crash-safe local delivery candidate

- **Original Assumption:** The alpha.13 TLS loopback path transferred a public store state in one request, so a receiver interruption would require a fresh transfer and had no durable progress record.
- **Observed Evidence:** The same TINP DATA path can carry bounded chunks. A receiver can atomically persist a manifest and chunk cursor, stop after chunk 7 of 14, restart from the same journal, commit through the existing convergence-store validators, and keep duplicate replay idempotent. Altered recorded chunks and a different manifest/state root under the same transfer id are rejected before durable replacement.
- **Reasoning:** A public-state journal addresses interruption and restart serialization without creating a second authority protocol. The manifest binds the transfer to the exact state root and payload digest; the journal is a delivery record, not a publication log, trusted clock or conflict authority.
- **New Decision:** Add `twni.authority-registry-resumable-transfer.v1` and `twni.authority-registry-resumable-transfer-journal.v1`, a test-only worker/demo/test path, and verifier evidence. Keep the payload public-state-only, enforce bounded chunk limits and atomic file replacement, and preserve unresolved conflicts.
- **Impact:** P05/P06/P09 gain bounded local crash-restart transfer evidence. This does not provide two physical hosts, production certificate custody, trusted time, online registry publication/revocation, lost-key recovery or distributed conflict consensus; no RCL Core, RNCS/RFE or AAF owner changes.
- **Rollback:** Remove the resumable transfer module, journal worker/demo/test, protocol row and alpha.14 evidence/docs; alpha.13 TLS and earlier TCP/replay/store behavior remain available.

## 15. Read-only OPP HTTP observation remains a policy adapter, not an OPP protocol rewrite

- **Original Assumption:** A legacy HTTP compatibility path could be added directly beside OPP negotiation and treated as general interoperability.
- **Observed Evidence:** The repository contains OPP CHP/RCP owner code but no externally accepted OPP consumer bridge. A narrow TINP adapter can reference the pinned OPP source, issue one explicit HTTPS `GET`, project JSON and bind wire/projected roots in a receipt while denying credentials, ambient proxies, redirects and authority.
- **Reasoning:** Keeping policy, request and receipt roots in TINP lets transport behavior be tested without copying or redefining OPP negotiation semantics. Deterministic local tests expose malformed field shapes, encoded paths, response metadata errors and size/media/status failures; one public GitHub REST observation is separately retained as a provider result.
- **New Decision:** Add `twni.opp-http-readonly-policy.v1`, `twni.opp-http-readonly-request.v1` and `twni.opp-http-readonly-receipt.v1` through a TINP-owned Node adapter and register the bounded behavior under P09/P15. Keep `authorityGranted:false` and retain an explicit OPP consumer bridge as the next gap.
- **Impact:** P09/P15 gain bounded read-only HTTP observation evidence. This does not create an RCL primitive, OPP third-party interoperability, public availability guarantee, production credential/certificate lifecycle, authority grant or K400 promotion; OPP CHP/RCP and external Owner boundaries remain unchanged.
