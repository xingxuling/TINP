...
Generality：大小写折叠 header 冲突拒绝、对象响应边界、`__proto__` 数据投影保护、重新计算 receipt root 后仍要求 PASS/FAIL_CLOSED 语义一致，这些是可跨 provider 复用的 fail-closed 约束；candidate absorption 仍为 TINP provider profile，不增加 RCL primitive、不改变 OPP/TINP/RNCS/AAF owner。Node 内置 `https.request` 与每请求显式 Agent 是 provider advantage；代理、证书、凭据、跨主机和恢复 lifecycle 仍由外部部署 Owner 负责。AI001 OpenAPI donor 只提供静态 source frontend，不改变这一 transport owner 决定。

Regression：adapter `10/10 PASS`，TINP full suite `184/184 PASS`；GitHub public read 的 policy/request/receipt/response roots 在 native transport 后复验通过；标准 `node:http.setGlobalProxyFromEnv` 指向不可用 loopback proxy 时 native 路径仍得到 HTTP 200。负例覆盖 ambient proxy、Node `NODE_USE_ENV_PROXY`/`NODE_OPTIONS`/`--use-env-proxy`、network no-retry、oversize/status/media/malformed/object-shape/projection、host/path/credential、duplicate/hidden header、prototype field 和 re-rooted forged receipt。

Affected K400 candidates：仅记录 EXPRESS/COMPILE/LOWER/EXECUTE/CORRECT/ROBUST 的本机候选证据；PERFORMANCE、AI_GENERATE 未评估，九门仍 `NOT_ADJUDICATED`。EVIDENCE 见 `evidence/OPP_HTTP_READONLY_HARDENING_2026-09-11.json`、`evidence/OPP_HTTP_READONLY_NATIVE_PROXY_CHECK_2026-09-11.json` 与 `docs/INTEGRATION_COURT_2026-09-11_OPP_HTTP_READONLY.md`；任意 runtime hook、`THIRD_PARTY_VERIFIED`、cross-host 或 production 仍不成立。
